...
void ui_init(void) {
  ...
  gpio_set_mode(&status_red_gpio, GPIO_OUTPUT_PWM);
  gpio_set_mode(&status_green_gpio, GPIO_OUTPUT_PWM);
  gpio_set_mode(&status_blue_gpio, GPIO_OUTPUT_PWM);
  ui_set_status(OFF, false);
  
  gpio_set_mode(&speaker_gpio, GPIO_OUTPUT_CTC);
  gpio_set_pwm_duty_cycle(&speaker_gpio, 0);
...
void ui_set_status(uint8_t colour, bool blink) {
  ...
  for (uint8_t i = 0; i < 3; i++) {
    gpio_set_pwm_duty_cycle(status_gpios[i], status_gpio_color_levels[colour][i]);
  }
  if (colour == OFF) {
...
static bool status_on_tick(time_t time, struct task* task) {
  ...
  if (status_state == LED_ON) {
    for (uint8_t i = 0; i < 3; i++) {
      gpio_set_pwm_duty_cycle(status_gpios[i], 0);
    } 
    status_state = LED_OFF;
  } else {
    for (uint8_t i = 0; i < 3; i++) {
      gpio_set_pwm_duty_cycle(status_gpios[i], colour_levels[i]);
    } 
    status_state = LED_ON;    
  }
  return true;
